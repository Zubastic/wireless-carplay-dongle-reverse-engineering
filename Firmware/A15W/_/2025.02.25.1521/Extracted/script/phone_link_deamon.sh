...
Usage() {
	echo "Usage: $0 CarPlay/AndroidAuto/HiCar/ICCOA/CarLife start/stop"
	exit 1
}
...
StartLinkDeamon() {
	...
	test -e $exitfile && (rm -f $exitfile;echo "Continue Run Link Deamon: $linktype")
...
RestartLinkDeamon() {
	if [ -e $lockfile ]; then
		echo "Restart Link Deamon: ${linktype}"
		EndLinkProcess
	fi
	exit 0
}
